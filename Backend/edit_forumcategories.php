...
if (isset($_POST['Forum'])) {
  if (UbahForumC($_POST) > 0) {
    $idf = $forum["Id_Forum"];
    header("Location:forumcategories_guru.php?Id=$idf");
  } else {
        ...
        <div class="add-docs tambah-tugas">
          <form action="" method="post" enctype="multipart/form-data" class="dropzone" id="image-upload">
            <div class="title d-flex">
              <p>Judul Forum</p>
              <input class="forum" value="<?= $forum['Judul']; ?>" name="forum" type="text" placeholder="Ketikkan judul forum disini">
            </div>
            <div class="title title-thread d-flex">
              <p class="forum-desk">Deskripsi</p>
              <div class="add-thread" id="add-thread">
                <textarea name="editor"><?php echo $forum['Deskripsi'] ?></textarea>
              </div>
            </div>
            <div class="click">
              <div class="btn-back">
              ...
              <div class="btn-start">
                <button type="submit" name="Forum" onclick="tambahConfirm()">Edit</button>
              </div>
            </div>
          </form>
        </div>
