...
$id = $_GET['Id'];
$s = query("SELECT * FROM slide WHERE id_slide = $id");

if (isset($_POST['slide'])) {
    if (editslide($_POST) > 0) {
                ...
                <form action="" method="post" enctype="multipart/form-data" class="dropzone" id="image-upload">
                    <div class="add-docs tambah-tugas">
                        ...
                        <div class="title tambah d-flex">
                            <p>Tambah File</p>
                            <input id="upfile" type="file" value="<?php echo $s['image'] ?>" name="NamaFile1" />
                            <input type="hidden" value="<?php echo $s['image'] ?>" name="image">
                        </div>
                    </div>
                    <div class="click d-flex" style="justify-content:center ; margin-bottom: 50px;">
                        ...
                        <div class="btn-start">
                            <button name="slide" type="submit" onclick="return confirm('Apakah Anda Yakin Ingin Mengubah Slide Ini?');">Edit Slide</button>
                        </div>
                    </div>
                </form>
