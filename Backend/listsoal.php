...
if (isset($_POST['tambah'])) {
  $conn = koneksi();
  header("Location: tambahsoal.php?Id=$Id");
}
...
<body>
  ...
  <div class="main-container">
    ...
    <div class="main-content-forum">
      ...
      <div class="box">
            <button class="soal" type="submit" name="tambah" onclick="location.href='tambahsoalpg.php?Id=<?= $Id?>'">Tambah Soal PG</button>
            <button class="soal" type="submit" name="tambah" onclick="location.href='tambahsoalessay.php?Id=<?= $Id?>'">Tambah Soal Essay</button>
          <?php if (empty($ujian)) : ?>
            <p class="empty">Belum ada soal yang tersedia untuk ujian ini</p>
          <?php endif ?>
          <?php  $i = 1; ?>
          <?php
          foreach ($ujian ?: [] as $f) : ?>
            <div class="list-soal">
              <p><b>Soal <?= $i++?></b></p>
              <p><?= $f["soal"]?></p>
              <input type="radio" name="opsi" id="" value="<?= $f["opsi1"]?>"><?= $f["opsi1"]?><br>
              <input type="radio" name="opsi" id="" value="<?= $f["opsi2"]?>"><?= $f["opsi2"]?><br>
              <input type="radio" name="opsi" id="" value="<?= $f["opsi3"]?>"><?= $f["opsi3"]?><br>
            </div>
          <?php endforeach ?>
          <?php
          foreach ($ujian1 ?: [] as $f) : ?>
            <div class="list-soal">
              <p><b>Soal <?= $i++?></b></p>
              <p><?= $f["soal"]?></p>
              <input type="text" name="" placeholder="input jawaban disini"  style="width: 20% !important;font-size: 12px;border: none; outline:none;background: none;border-bottom: 1px #082C73 solid;color:#dddd"; readonly id="">
            </div>
          <?php endforeach ?>
