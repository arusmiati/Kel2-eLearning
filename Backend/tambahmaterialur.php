...
if (isset($_POST['alur'])) {
  if (TambahMateri($_POST) > 0) {
    ...
    echo 'Alur Gagal Ditambahkan';
  }
}

$alur = query("SELECT * FROM alur  WHERE Id_Alur = '$Id'");
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title>Tambah Materi Alur</title>
      ...
      <div class=" doc-content">
        <div class="doc-header">
          <div class="d-flex">
            ...
            <h1>Tambah Materi Alur</h>
          </div>
        </div>
        <div class="add-docs tambah-alur">
          <form action="" method="post" enctype="multipart/form-data">
            <div class="title d-flex" id="link-tautan">
              <p>Judul</p>
              <input name="judul" type="text" placeholder="Ketikkan Judul disini">
            </div>
            <div class="title d-flex" id="link-tautan">
              <p>Deskripsi Singkat</p>
              <input name="deskripsi" type="text" placeholder="Ketikkan Deskripsi disini (optional)">
            </div>
            <div class="add-file d-flex">
              <p>Tambah Materi</p>
              <input id="file" type="file" value="upload" name="file" multiple />
            </div>
            <div class="click">
              <div class="btn-back">
                <button onclick="location.href='detailpertemuan_guru.html'">Kembali</button>
              </div>
              <div class="btn-start">
                <button type="submit" name="alur" onclick="return confirm('Apakah Anda Yakin Ingin Menambahkan Materi Ini?');">Tambah</button>
              </div>
            </div>
          </form>
        </div>
  ...
  <script>
    ...
    function tambahConfirm() {
      if (c == 0) {
        ...
        setTimeout(() => {
          window.location.href = 'detailpertemuan_guru.html'
        }, 3000);
        c = 1;
      } else {
