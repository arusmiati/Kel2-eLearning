...
if (isset($_POST['tautan'])) {
  if (edittautan($_POST) > 0) {
    header("Location:tautanguru.php?Id=$id");
  } else {
    echo 'Tautan Gagal Ditambahkan';
  }
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <div class="main-container">
    ...
    <div id="confirm-box-tambah">
      <h1>Tautan<br> Berhasil Ditambahkan</h1>
    </div>
    <div class="tabel">
        ...
        <div class="add-docs tambah-tugas">
          <form action="" method="post">
            <div class="title d-flex">
              <p>Judul</p>
              <input value="<?= $tautan["Judul"] ?>" name="judul" type="text" placeholder="Ketikkan judul tautan disini">
            </div>
            <div class="title d-flex" id="link-tautan">
              <p>Link URL</p>
              <input value="<?= $tautan["Link"] ?>" name="url" type="text" placeholder="Salin link URL disini">
            </div>
            <div class="click">
              <div class="btn-back">
                <button onclick="location.href='tautanguru.html'">Kembali</button>
              </div>
              <div class="btn-start">
                <button type="submit" name="tautan" onclick="tambahConfirm()">Edit</button>
              </div>
            </div>
          </form>
        </div>
