...
if (isset($_POST['TambahForum'])) {
  $conn = koneksi();
  header("Location: tambahforum.php?Id=$Id_Mapel");
}

$forum = querys("SELECT * FROM forum  WHERE Id_Mapel = '$Id_Mapel'");
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="main-content-forum">
      ...
      <div class="box">
        <ul class="forum-box">
          ...
          <?php endif ?>
          <?php $i = 1;
          foreach (array_reverse($forum) ?: [] as $f) : ?>
            <li class="forum-item d-flex">
              <a href="forumcategories_guru.php?Id=<?= $f['Id_Forum']; ?>">
                <p><b><?= $f["Nama_Forum"]; ?></b></p>
              </a>
              <div class="dropdown">
                ...
                <div id="drop-history" class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                  <a class="dropdown-item" href="editforum.php?Id=<?= $f['Id_Forum']; ?>">Edit Forum</a>
                  <a class="dropdown-item" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Forum Ini?');" href="hapusforum.php?id=<?= $f['Id_Forum']; ?>">Delete Forum</a>
                </div>
              </div>
            </li>
          <?php endforeach ?>
