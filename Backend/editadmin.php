            ...
            <div class="tabel-content doc-content">
                ...
                <div class="add-docs tambah-tugas">
                    <form action="" method="post" enctype="multipart/form-data" class="dropzone" id="image-upload">
                        ...
                        $Id = $_GET['Id'];
                        $siswa = query("SELECT * FROM user_admin WHERE Id = $Id");

                        ?>
                        <div class="title d-flex">
                            <p>Nama Admin</p>
                            <input name="nama" value="<?php echo $siswa["Nama"] ?>" type="text" placeholder="Ketikkan nama admin disini" required>
                        </div>
                        <div class="title d-flex">
                            <p>Nip</p>
                            <input name="nip" value="<?php echo $siswa["Nip"] ?>" type="text" placeholder="Ketikkan nip admin disini" required>
                        </div>
                        <div class="title d-flex">
                            <p>Posisi</p>
                            <input name="posisi" value="<?php echo $siswa["Posisi"] ?>" type="text" placeholder="Ketikkan posisi admin disini (Mis: Administrator)" required>
                        </div>
                        <div class="title d-flex">
                            <p>TTL</p>
                            <div class="waktu">
                                <input name="tempat" value="<?php echo $siswa["Tempat"] ?>" style="width:200px;" type="text" placeholder="Ketikkan tempat lahir admin disini">
                                <input name="tanggal" type="date" value="<?php echo $siswa["Tanggal"] ?>" placeholder="Ketikkan tanggal lahir admin disini" required>

                            </div>
                        </div>
                        <div class="title d-flex">
                            <p>Alamat</p>
                            <input name="alamat" value="<?php echo $siswa["Alamat"] ?>" type="text" placeholder="Ketikkan alamat admin disini (opsional)">
                        </div>
                        <div class="title d-flex">
                            <p>Email</p>
                            <input name="email" type="text" value="<?php echo $siswa["Email"] ?>" placeholder="Ketikkan email admin disini (opsional)">
                        </div>
                        <div class="title d-flex">
                            <p>Username</p>
                            <input name="username" type="text" value="<?php echo $siswa["Username"] ?>" placeholder="Ketikkan username admin disini" required>
                        </div>
                        <div class="title d-flex">
                            <p>Password</p>
                            <input name="password" type="text" value="<?php echo $siswa["Password"] ?>" placeholder="Ketikkan password admin disini" required>
                        </div>
                        <div class="title d-flex">
                            <p>Profil (Opsional)</p>
