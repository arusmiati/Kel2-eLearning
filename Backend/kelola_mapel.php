...
if (isset($_POST['save'])) {
    ...
    for ($i = 0; $i < count($checkbox); $i++) {
        $del_id = $checkbox[$i];
        mysqli_query($conn, "DELETE FROM tambah_kelas WHERE Id_TambahKls ='" . $del_id . "'");
            ...
            <div class="tabel-content">
                <div class="doc-header kelola d-flex">
                    <div class="head d-flex">
                        <div class="icon"><img src="img/mapel.png" alt=""></div>
                ...
                <form action="" method="post" id="delete_form">
                    <div class="del-sel">
                        ...
                        <button onclick="location.href='tambahmapeladmin.php'" style="background: #58BAAB;color: white;outline: none;cursor: pointer;"><a style=" text-decoration: none; color: white;" href="tambahmapeladmin.php">Tambah Mapel</a></button>
                    </div>
                    ...
                    <div class="item-tabel">
                        <table>
                            ...
                            <?php
                            $user = querys("SELECT * FROM tambah_kelas");
                            if (isset($_POST['cari'])) {
                                $user = cari_mapel($_POST['keyword']);
                            }
                            foreach ($user as $u) : ?>
                                <tr class="item items">
                                    <td><input type="checkbox" id="checkItem" name="check[]" value="<?= $u["Id_TambahKls"] ?>"></td>
                                    <td><?php echo $u["Nama_Kelas"] ?></td>
                                    <td>ES<?= $u["Id_TambahKls"] ?></td>
                                    <td><?= ($u["Semester"]) ?></td>
                                    <td><?= $u["Nama_Guru"] ?></td>
                                    <td>
                                        <div class="opsi">
                                            <button class="edit" onclick="location.replace='editmapeladmin.php?Id=<?= $u["Id_TambahKls"] ?>'"><a href="editmapeladmin.php?Id=<?= $u["Id_TambahKls"] ?>">edit</a></button>
                                        </div>
                                    </td>
